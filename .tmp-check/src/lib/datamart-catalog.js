 ...
 */
function toDataSize(capacity, mb) {
    if (capacity >= 1) {
 ...
 */
function makeBundleId(networkId, dataSize) {
...
async function fetchDataMartCatalog() {
    if (!DATAMART_API_KEY) {
...
export async function syncDataMartCatalog() {
    ...
    const NETWORK_KEYS = ["YELLO", "TELECEL", "AT_PREMIUM"];
    ...
    const networkDocs = (await fsQuery("networks"));
    const codeToNetworkId = new Map();
    ...
    const existingDocs = (await fsQuery("bundles"));
