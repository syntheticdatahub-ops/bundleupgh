...
const privateKey = (_a = envVars["FIREBASE_PRIVATE_KEY"]) === null || _a === void 0 ? void 0 : _a.replace(/\\n/g, "\n");
...
function httpsRequest(options, body) {
    return new Promise((resolve, reject) => {
        const req = https.request(Object.assign(Object.assign({}, options), { agent }), (res) => {
            let data = "";
...
async function setDoc(token, collection, docId, fields) {
    ...
    for (const [k, v] of Object.entries(Object.assign(Object.assign({}, fields), { createdAt: now, updatedAt: now }))) {
        if (typeof v === "string")
...
async function addDoc(token, collection, fields) {
    const now = new Date().toISOString();
    const firestoreFields = {};
    for (const [k, v] of Object.entries(Object.assign(Object.assign({}, fields), { createdAt: now, updatedAt: now }))) {
        if (typeof v === "string")
...
async function seed() {
    ...
    for (const bundle of bundles) {
        await setDoc(token, "bundles", bundle.id, Object.assign(Object.assign({}, bundle), { id: bundle.id }));
